...
pub fn handle(args: Vec<String>, cancellation_token: &CancellationToken) -> Result<()> {
    ...
    let config = commons::resolve_config_from_context_or_session(Some(link_args.context), cancellation_token)?;
    ...
    let (new_parent_uuid, new_parent_qualified_name) =
        context_resolver::resolve_context(new_parent_context, &index, cancellation_token).with_context(|| {
            anyhow!(
