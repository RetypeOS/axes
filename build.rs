...
fn load_translations(lang: &str) -> HashMap<String, String> {
    ...
    let fallback_content = fs::read_to_string(fallback_path)
        .unwrap_or_else(|e| exit_with_error(format!("Failed to read fallback language file '{}': {}", fallback_path, e)));
    let mut translations: HashMap<String, String> = toml::from_str(&fallback_content)
        .unwrap_or_else(|e| exit_with_error(format!("Failed to parse TOML from '{}': {}", fallback_path, e)));
        ...
        match fs::read_to_string(&lang_file_path) {
            Ok(content) => {
                let specific_translations: HashMap<String, String> = toml::from_str(&content)
                    .unwrap_or_else(|e| exit_with_error(format!("Failed to parse TOML from '{}': {}", lang_file_path, e)));
...
fn generate_macro_file(translations: &HashMap<String, String>) {
    let out_dir = env::var("OUT_DIR")
        .unwrap_or_else(|_| exit_with_error("The OUT_DIR environment variable was not set by Cargo."));
    ...
    writeln!(macro_code, "#[macro_export]\nmacro_rules! t {{").unwrap();
    ...
    fs::write(&dest_path, macro_code)
        .unwrap_or_else(|e| exit_with_error(format!("Failed to write generated code to '{:?}': {}", dest_path, e)));
}